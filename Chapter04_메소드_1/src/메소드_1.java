/*
		...
		1. 사용 목적 : - 가독성 , - 재사용성 , - 속도 (최적화)
					 ------------------
					 		 |
					 	메소드 사용 이유
					 	
			1) 분석 (가독성) => 수정 (유지보수) , 오류처리
			...
			2) 다른 클래스와 연결 (메세지)
			  ...
			  => Scanner... (사용자 정의)
			  		scan.nextInt()
			  			 ---------> 메소드 (입력된 정수 읽기)
			  		Sysout.printin() >  화면에 출력
			  		trim() , length() 등
			  		=> 메소드는 한자리 기능만 수행이 가능하게 만듦 (사용하기 편함)
			  			1) 입력
		...
		2. 메소드 형식
			...
			default					=> 요구사항 분석
			protected	final : 상속이 불가능 (수정이 불가능)
								=> Math , String , System...
			{
				구현부
			}
			...
			==> 선언부 | 구현부
				 ...
				 *** 약속 사항 ***
				- 소문자로 시작
				- 두개의 단어가 있는 경우 두번째 단어 첫자는 대문자 || _ 사용
				- 데이터 저장 : set변수명()...
				- 데이터 읽기 : get변수명()...
		...
		3. 메소드 제작시 고민
		   ...
		   ----------------------------------------
		   			X					X
		   			Sysout.println() => 다음줄에 출력 (거의 빈도수가 없음)
		   ----------------------------------------
...
public class 메소드_1 {
	static void a()
	{
		...
		return; // 생략 가능 => 값을 보내줄때 한개만 사용 가능
	}
	// 시작점
	public static void main(String[] args) {
