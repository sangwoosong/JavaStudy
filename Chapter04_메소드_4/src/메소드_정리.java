/*
		...
		=> 매개변수 전송
			int c=add(10 , 20) => 호출시에는 매개변수값을 전송
					   |	|
			int add(int a, int b) => 임시 메모리 공간 (호출시에 할당되고 끝나면 사라짐) => 지역변수의 일종
			{
				return a+b; => 30 => c에 저장
			}
		...
		메소드 정의
		---------
		1. 특정 작업을 수행하는 명령문 집합
						   -----> 변수 / 연산자 / 제어문
						   
		2. 메소드를 호출할 경우에만 매개변수가 메모리 할당을 함
			메소드는 호출할때마다 처음부터 return이 있는 곳까지 실행이 됨
...
public class 메소드_정리 {
	...
	public static void main(String[] args) {
		...
		m1.title="aaa";
		m1.actor="bbb";
		Movie m2=new Movie(); // 위에와 다른 메모리에 저장 (new 사용) => 하지만 설계는 하나 (클래스)
		m2.title="ccc";
		m2.actor="ddd";
