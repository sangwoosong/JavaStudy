/*
	...
	2. 데이터 관리를 위한 사용자 정의 데이터형
		...
		- 클래스의 변수의 종류 : 클래스 영역에 선언하는 변수 (멤버 변수) => heap에 저장
								- 인스턴스변수 (객체) => new를 사용할때마다 따로 저장
								  ----------> heap 영역에 저장
								- 클래스변수 => 공유변수 => static
											=> Method area
			자바의 메모리
			...
			--------------------------------------------------
			인스턴스변수				- new : 새로운 공간이 생성 => 클래스 전체 사용
...
/*
		파일명은 .java
			   ...
			   => 클래스명과 동일 (대소문자 구분)
			   A.java, a.java
		한 파일 안에 여러개의 클래스를 사용할 수 있음
		------------------------------------
		class A
		public class B
		--------------> public class는 한 파일 안에 1개만 사용이 가능
					   ------------> 파일명이 됨
		class A
		class B
		-------- 저장명은 A.java, B.java
